...
uint16_t LevelController::serialize(void* buffer, uint16_t size)
{
	...
	uint16_t mySize = sizeof(debounceDelayLow) + sizeof(debounceDelayHigh) + sizeof(refillTimeout);
	if (mySize <= size){
		...
		WebServer::ConnectionType type)
{
	...
	else
	{
		...
		parser->processSingleTemplate(&templateFile, templateStrings,
				replacementStrings, 4, server);
		templateFile.close();
	}
	return true;
}
